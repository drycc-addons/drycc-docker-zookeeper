...
SHELL_SCRIPTS = $(shell find ${ZOOKEEPER_VERSION}/debian/ -name '*.sh') $(wildcard *.sh)
...
docker-build: check-docker
	docker build ${DOCKER_BUILD_FLAGS} -t ${IMAGE} ${ZOOKEEPER_VERSION}/debian
	docker tag ${IMAGE} ${MUTABLE_IMAGE}

docker-buildx: check-docker
	docker buildx build --platform ${PLATFORM} -t ${IMAGE} ${ZOOKEEPER_VERSION}/debian --push
